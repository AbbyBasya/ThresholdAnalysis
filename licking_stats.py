...
#from parse_data_v2 import Mouse_200218
...
#load_path = 'C:/Abby Behavior Data/Pilot2/experiment_data_2022_1_alternating blocks go_nogo/parsed_dataframe_pickle/{0}_stats.pickle'.format(mouse_id)
load_path = 'C:/Abby Behavior Data/Pilot2/experiment_data_2022_1_alternating blocks go_nogo/parsed_dataframe_pickle/test_analysis_stats.pickle'
...
trial_type = 'go'
is_save = True
save_dir = 'C:/Abby Behavior Data/Pilot2/experiment_data_2022_1_alternating blocks go_nogo/figures2'
...
for index, day in enumerate(df.all_days[:sum(df.is_cond_day)]):
    ...
    ax[index, 0].set_title('{} {}'.format(day, 'condition' if df.is_cond_day[index] else 'degradation'), y=0.95,
                           fontsize=fsize)
...
for index, day in enumerate(df.all_days[sum(df.is_cond_day):]):
    ...
    ax[index + offset, 1].set_title(
        '{} {}'.format(day, 'condition' if df.is_cond_day[index + sum(df.is_cond_day)] else 'degradation'), y=0.95,
        fontsize=fsize)
    ax[index + offset, 1].spines["top"].set_visible(False)
    ax[index + offset, 1].spines["right"].set_visible(False)
    ax[index + offset, 1].set_xlabel(
        'Time after odor onset (s)' if index == len(df.all_days) - sum(df.is_cond_day) - 1 else '', fontsize=fsize)
...
is_save = True
save_dir = 'C:/Abby Behavior Data/Pilot2/experiment_data_2022_1_alternating blocks go_nogo/figures'
...
for index in range(len(all_days)):
    ...
    day = all_days[index]
    hist = df.df_trials_lick[day].hist(figsize=(15, 2), column='rate', by='trialtype', layout=(1, 4), bins=20,
                                       density=True,
                                       range=(0, 6), edgecolor='black',
                                       facecolor='#c47898' if df.is_cond_day[index] else '#599B83',
...
fig, ax = plt.subplots(nrows=int(max(sum(df.is_cond_day), len(df.all_days) - sum(df.is_cond_day))), ncols=2,
                       sharex=True, sharey=True, figsize=(10, 20), dpi=80)
fsize = 14
for i in range(len(df.all_days)):
    ...
    both_trials = df.df_trials_lick[day][
        index_both]  # a dataframe of a particular day that only contains go and go_omit trials
    ...
    sns.distplot(array, color="dodgerblue", ax=ax[rows, cols], label="{}".format(df.all_days[i]), rug=True,
                 rug_kws={"color": '#c47898' if df.is_cond_day[i] else "#599B83"},
                 kde_kws={"color": "k", "lw": 3}, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1,
                                                            "color": '#c47898' if df.is_cond_day[i] else "#599B83"})
    ...
    ax[rows, cols].set_xlabel(
        'Time after odor onset (s)' if i in [sum(df.is_cond_day) - 1, len(df.all_days) - 1] else '', fontsize=fsize)
...
for index, row in Stats_df.iterrows():
    ...
    ax[0].plot(x, np.array(row['Hit']), 'o-', linewidth=3, alpha=0.8, markeredgecolor='k', markersize=markersize,
               color='#a163ab', label='hit' if index == 1 else '')
    ax[1].plot(x, row['Correct_rej'], 'o-', linewidth=3, alpha=0.8, markeredgecolor='k', markersize=markersize,
               color='#e39a62', label='correct rejection' if index == 1 else '')
    ...
    ax[2].plot(x, row['Go_lick'], 'o-', linewidth=3, alpha=0.8, markeredgecolor='k', markersize=markersize,
               color='#c47898' if df.is_cond_day[index] else '#599B83')
    ax[2].set_ylabel('Lick rate to Odor\n over delay')
    ax[3].plot(x, row['Go_latency'], 'o-', linewidth=3, alpha=0.8, markeredgecolor='k', markersize=markersize,
               color='#c47898' if df.is_cond_day[index] else '#599B83')
    ax[3].set_ylabel('Latency to lick')
    ax[4].plot(x, row['Background_lick'], 'o-', linewidth=3, alpha=0.8, markeredgecolor='k', markersize=markersize,
               color='#c47898' if df.is_cond_day[index] else '#599B83')
...
# save data by pickle
# ****************
save_path = 'C:/Abby Behavior Data/Pilot2/experiment_data_2022_1_alternating blocks go_nogo/parsed_dataframe_pickle'
